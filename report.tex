...
Not everything has a strict surface. Sphere tracing expects the world to be made of solid objects.  But if you try to render smoke, hair, or fire, there's no real 'surface' to hit. Moreover, the volume rendering naturally handles all the fluffy and transparent stuff that SDFs struggle with.
...
In \textbf{uniform} sampling a whole object can slip right between two samples and get missed completely, but the objects we do hit come out nice and full with no holes.
In \textbf{stratified} sampling we almost never miss an object entirely, but since the samples land in different places on neighboring rays, we end up with more holes and a grainier look.
...
\begin{itemize}
    \item Our model only uses the 3D position $\mathbf{x}=(x,y,z)$ to guess the color. The original NeRF also looks at the viewing angle $\mathbf{d}$. This means the original model can render reflections/shiny surfaces.
